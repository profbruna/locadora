...
class locacaoProdController extends CI_Controller {
    ...
    public function listar() {
        ...
        $dados['paginacao'] = criaPaginacao('locacaoProdController', $this->locacaoProdModel->contarTudo(), $this->uri->segment(3), 3);
    ...
    function inserir_produto() {
        ...
        $dados['data_devolucao'] = implode('-', array_reverse(explode('/', $dados['data_devolucao'])));
    ...
    function alterar_produto($codigo) {
        ...
        $dados['prod'] = $this->locacaoProdModel->buscar_pelo_codigo($codigo);
    ...
    function excluir_produto($codigo) {
        $this->load->model('locacaoProdModel');
        $num = $this->locacaoProdModel->contarTudo($codigo);
        if ($num == 0) {
            if ($this->locacaoProdModel->excluir()) {
